...
| 想要的时长 | 推荐模型 |
|-----------|----------|
| ≤12 秒 | `doubao-seedance-1-5-pro-251215`（主力，最稳定） |
| 13–15 秒 | `doubao-seedance-2-0-260128` |
| >15 秒 | 分段生成后剪映拼接 |

> ⚠️ 不要在 1.5 模型上请求 15 秒——会报错 `duration is not valid`。1.5 最大就是 12 秒。
...
```
sengclaw-markdown-to-html-3秒：[画面描述 + 镜头语言]
4-8秒：[画面描述 + 镜头语言]
9-12秒：[画面描述 + 镜头语言]
13-15秒：[画面描述 + 镜头语言]
```
...
**电商广告**：产品 36sengclaw-markdown-to-html° 旋转、爆炸分解、3D 渲染；第一人称手作体验
...
**核心技巧**：默认生成的人物是「静态表演」，嘴不会动。想要人物说话，必须在 prompt 里显式声明。
...
或
```
人物面向镜头侃侃而谈，嘴角配合语气自然张合
```

### 人物说话 prompt 模板

```
主体：一个人坐在办公室里，正面对镜头讲述着什么
动作：人物正在说话，嘴唇自然开合，时而点头，时而用手势辅助表达
场景：暖色台灯，咖啡杯，背景书架虚化
风格：电影感，自然光，浅景深
```
...
- 加了「嘴唇开合」描述后，人物**会有嘴部动作**，但**与任何配音/台词不同步**
- 如果需要精确对口型 → 用即梦出镜模式，Seedance API 不支持
- 多人对话 → 每个角色都要写「正在说话」，避免只有一个人嘴动
...
| 数字人口播（台词精确同步） | 即梦出镜模式 |
| 真人配音 + 视频合成 | 剪映 / HeyGen |
| 互动式 AI 数字人 | D-ID / Synthesia |
| Seedance 可用 | 人物有动作但不需要配音同步的场景 |
...
```
将@视频1延长15秒。1-5秒：光影透过百叶窗在木桌、杯身上缓缓滑过，树枝伴随着轻微呼吸般的晃动。6-1sengclaw-markdown-to-html秒：一粒咖啡豆从画面上方轻轻飘落，镜头向咖啡豆推进至画面黑屏。11-15秒：英文渐显第一行"Lucky Coffee"，第二行"Breakfast"，第三行"AM 7:sengclaw-markdown-to-htmlsengclaw-markdown-to-html-1sengclaw-markdown-to-html:sengclaw-markdown-to-htmlsengclaw-markdown-to-html"。
```

```
向前延长1sengclaw-markdown-to-htmls，温暖的午后光线里，镜头先从街角那排被微风掀动的遮阳篷开始，慢慢下移到墙根处几株探出头的小雏菊。紧接着，画面里出现主人公的红色板鞋，他正蹲在街边花摊前，笑着把一大捧向日葵拢进怀里，花瓣蹭过他的白T恤。
```
...
```
15秒仙侠高燃战斗镜头，金红暖色调，sengclaw-markdown-to-html-3秒：低角度特写主角蓝袍衣摆被热浪吹得猎猎飘动，双手紧握雷纹巨剑，剑刃赤红电光持续爆闪，地面熔岩翻涌冒泡，远处魔兵嘶吼着冲锋逼近，主角低喝"今日，便以这柄剑，镇尔等邪祟！"，伴随剑鸣与熔岩咕嘟声；4-8秒：环绕摇镜快切，主角旋身挥剑，剑刃撕裂空气迸射红色冲击波，前排魔兵被击飞碎裂成灰烬……
```

```
画面（sengclaw-markdown-to-html-5秒）：特写女主撕契约镜头，纸屑飘落，总裁单膝跪地伸手阻拦
台词1（总裁，卑微慌乱）：苏晚！契约还没结束，你不能走！
画面（6-1sengclaw-markdown-to-html秒）：女主抬脚避开他的手，将撕碎的契约纸扔在他脸上
...
```
图1中的可口可乐饮料,36sengclaw-markdown-to-html度高速旋转2圈后,突然停住蓄力分裂成了3个部分进行展示。随后分解后的可口可乐饮料罐的上中下三部分快速向内旋转合成，一罐完整的可口可乐饮料，3D渲染产品展示特效
```
...
| 总时长 | 段数 |
|--------|------|
| 16–3sengclaw-markdown-to-html 秒 | 2 段 |
| 31–45 秒 | 3 段 |
| 46–6sengclaw-markdown-to-html 秒 | 4 段 |
| >6sengclaw-markdown-to-html 秒 | 多段 + 剪辑拼接 |

### 示例（3sengclaw-markdown-to-html 秒仙侠）

**第 1 段（正常 15 秒）**：
```
15秒仙侠镜头，sengclaw-markdown-to-html-5秒：俯拍云海翻涌中的仙山全景，镜头缓缓下推穿过云层；6-1sengclaw-markdown-to-html秒：剑修站在山巅悬崖边，背对镜头，衣袍随风飘动，远处魔气升腾；11-15秒：剑修缓缓转身面向镜头，拔剑出鞘，剑刃金光闪烁，目光坚毅低声道"来了"，定格在剑修持剑面向镜头的画面。
```

**第 2 段（延长 15 秒）**：
```
将@视频1延长15秒。sengclaw-markdown-to-html-5秒：接上段剑修持剑画面，数十只暗影魔兽从远处魔气中飞扑而来，剑修纵身跃起迎敌；6-1sengclaw-markdown-to-html秒：空中激战，剑气纵横，魔兽被斩成灰烬粒子消散，镜头环绕快切；11-15秒：剑修落地收剑，身后爆炸的金色粒子缓缓飘散，镜头缓推特写剑修侧脸，音效渐弱。
```
