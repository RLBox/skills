---
---
name: sengclaw-markdown-to-html
description: >-
  将 Markdown 转换为带样式的 HTML，支持微信兼容主题。支持代码高亮、数学公式、PlantUML、脚注、警告块、信息图，以及外链可选转为底部引用。当用户说"markdown to html"、"md 转 html"、"微信外链转底部引用"，或需要从 Markdown 生成带样式 HTML 时使用。
version: 1.56.1
metadata:
  openclaw:
    requires:
...
test -f .sengclaw-skills/sengclaw-markdown-to-html/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-markdown-to-html/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-markdown-to-html/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-markdown-to-html/EXTEND.md` | All directories |
...
If CJK content detected AND `sengclaw-format-markdown` skill is available:
...
**If user agrees**: Invoke `sengclaw-format-markdown` skill to format the file, then use formatted file as input.
...
3. `sengclaw-post-to-wechat` EXTEND.md `default_theme` (cross-skill fallback)
...
```bash
# Check sengclaw-post-to-wechat EXTEND.md for default_theme (user-level path)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-post-to-wechat/EXTEND.md" && grep -o 'default_theme:.*' "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-post-to-wechat/EXTEND.md"
```
