...
const CACHE_DIR_NAME = "url-to-markdown-cache";
...
export function resolveUrlCacheDir(): string {
  const override = process.env.URL_CACHE_DIR?.trim();
  if (override) return path.resolve(override);
  return path.join(os.homedir(), ".cache", APP_DATA_DIR, CACHE_DIR_NAME);
}
