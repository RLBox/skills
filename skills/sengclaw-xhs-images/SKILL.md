---
name: sengclaw-xhs-images
description: 生成小红书信息图系列，支持 11 种视觉风格和 8 种版式。将内容拆分为 1-10 张卡通风格图片，针对小红书互动效果优化。当用户提到"小红书图片"、"XHS images"、"RedNote 信息图"、"小红书种草"，或需要面向中文社交平台的图文时使用。
version: 1.56.1
metadata:
  openclaw:
---
...
/sengclaw-xhs-images posts/ai-future/article.md

# Specify style
/sengclaw-xhs-images posts/ai-future/article.md --style notion

# Specify layout
/sengclaw-xhs-images posts/ai-future/article.md --layout dense

# Combine style and layout
/sengclaw-xhs-images posts/ai-future/article.md --style notion --layout list

# Use preset (style + layout shorthand)
/sengclaw-xhs-images posts/ai-future/article.md --preset knowledge-card

# Preset with override
/sengclaw-xhs-images posts/ai-future/article.md --preset poster --layout quadrant
...
/sengclaw-xhs-images --style bold --layout comparison
[paste content]
```
...
test -f .sengclaw-skills/sengclaw-xhs-images/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-xhs-images/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-xhs-images/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-xhs-images/EXTEND.md` | All directories |

| Result | Action |
|--------|--------|
| Found | Read, parse, display summary → Continue to Step 1 |
| Not found | ⛔ BLOCKING: Run first-time setup ONLY (see below) → Save to user-level path → Then Step 1 |
