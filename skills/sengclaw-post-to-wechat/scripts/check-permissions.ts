...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.sengclaw-skills', '.env');
  const userEnv = path.join(os.homedir(), '.sengclaw-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== sengclaw-post-to-wechat: Permission & Environment Check ===\n');
