...
export function loadExtendConfig(): Partial<ExtendConfig> {
  const paths = [
    path.join(process.cwd(), ".sengclaw-skills", "sengclaw-markdown-to-html", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(homedir(), ".config"),
      "baoyu-skills", "sengclaw-markdown-to-html", "EXTEND.md"
    ),
    path.join(homedir(), ".sengclaw-skills", "sengclaw-markdown-to-html", "EXTEND.md"),
  ];
