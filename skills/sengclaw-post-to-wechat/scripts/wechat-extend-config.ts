...
function parseWechatExtend(content: string): WechatExtendConfig {
  ...
  for (const raw of lines) {
    ...
    switch (key) {
      ...
      case "preview_wxname": config.preview_wxname = val; break;
    }
  }
...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    path.join(process.cwd(), ".sengclaw-skills", "sengclaw-post-to-wechat", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"),
      "sengclaw-skills", "sengclaw-post-to-wechat", "EXTEND.md"
    ),
    path.join(os.homedir(), ".sengclaw-skills", "sengclaw-post-to-wechat", "EXTEND.md"),
  ];
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or .sengclaw-skills/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdEnvPath = path.join(process.cwd(), ".sengclaw-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".sengclaw-skills", ".env");
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.sengclaw-skills/.env (${prefixedKeyLabel})`, cwdEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
      buildCredentialSource(`~/.sengclaw-skills/.env (${prefixedKeyLabel})`, homeEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<cwd>/.sengclaw-skills/.env", cwdEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.sengclaw-skills/.env", homeEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
