...
```bash
# Recommended: save draft + send preview to phone, then publish after confirmation
${BUN_X} ./scripts/wechat-article.ts --markdown article.md --theme default --cover cover.jpg --submit --preview-wxname SengMitnick

# Save draft only (no preview)
${BUN_X} ./scripts/wechat-article.ts --markdown article.md --theme default --cover cover.jpg --submit

# Open preview dialog manually (no auto-send)
${BUN_X} ./scripts/wechat-article.ts --markdown article.md --submit --preview
...
| `--cover <path>` | Cover image path (required for WeChat preview/publish) |
| `--submit` | Save as draft to WeChat (required for preview and publish) |
| `--preview` | Open 发送预览 dialog after saving (manual fill) |
| `--preview-wxname <id>` | Auto-send preview to WeChat ID/phone/QQ (implies `--preview`). Reads from EXTEND.md `preview_wxname` if not set via CLI |
