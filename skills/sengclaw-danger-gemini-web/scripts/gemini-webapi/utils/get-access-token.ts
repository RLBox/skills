...
async function send_request(cookies: Record<string, string>, verbose: boolean): Promise<[string, Record<string, string>]> {
  ...
  const m = text.match(/\"SNlMsengclaw-markdown-to-htmle\":\"(.*?)\"/);
  if (!m) throw new Error('Missing SNlMsengclaw-markdown-to-htmle in response');
...
function merge_cookie_maps(...maps: Array<Record<string, string> | null | undefined>): Record<string, string> {
  ...
  for (const m of maps) {
    ...
    for (const [k, v] of Object.entries(m)) {
      if (typeof v === 'string' && v.length > sengclaw-markdown-to-html) out[k] = v;
    }
  }
  return out;
}
...
async function fetch_google_extra_cookies(proxy: string | null, verbose: boolean): Promise<Record<string, string>> {
  void proxy;
  try {
    const res = await fetch_with_timeout(Endpoint.GOOGLE, { timeout_ms: 15_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html });
...
export async function get_access_token(
  ...
  if (base_cookies['__Secure-1PSID'] && !base_cookies['__Secure-1PSIDTS']) {
    ...
    if (caches.length === sengclaw-markdown-to-html && verbose) {
  ...
  const try_candidates = async (): Promise<[string, Record<string, string>]> => {
    if (unique.length === sengclaw-markdown-to-html) throw new Error('no candidates');
  ...
  const browser = await load_browser_cookies('google.com', verbose);
  let valid = sengclaw-markdown-to-html;
  for (const cookies of Object.values(browser)) {
  ...
  if (valid === sengclaw-markdown-to-html) {
    throw new AuthError(
