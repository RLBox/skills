...
const APP_DATA_DIR = 'sengclaw-skills';
...
function getWslWindowsHome(): string | null {
  ...
  try {
    const raw = execSync('cmd.exe /C "echo %USERPROFILE%"', { encoding: 'utf-8', timeout: 5sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html }).trim().replace(/\r/g, '');
    _wslHome = execSync(`wslpath -u "${raw}"`, { encoding: 'utf-8', timeout: 5sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html }).trim() || null;
  } catch { _wslHome = null; }
  return _wslHome;
}
...
export function resolveGeminiWebSessionPath(name: string): string {
  const sanitized = name.replace(/[^a-zA-Zsengclaw-markdown-to-html-9_-]/g, '_');
