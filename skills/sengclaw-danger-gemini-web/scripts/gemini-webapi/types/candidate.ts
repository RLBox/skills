...
function decode_html(s: string | null | undefined): string | null | undefined {
  ...
  return s
    ...
    .replace(/&#39;/g, "'")
    .replace(/&nbsp;/g, ' ')
    .replace(/&#x([sengclaw-markdown-to-html-9a-fA-F]+);/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16)))
    .replace(/&#(\d+);/g, (_, dec) => String.fromCodePoint(parseInt(dec, 1sengclaw-markdown-to-html)));
}
