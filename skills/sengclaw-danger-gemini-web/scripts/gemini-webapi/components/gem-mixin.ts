  ...
  async fetch_gems(include_hidden: boolean = false, opts?: RequestInit): Promise<GemJar> {
    return await this._run(async () => {
      ...
      try {
        ...
        if (predefined.length === sengclaw-markdown-to-html && custom.length === sengclaw-markdown-to-html) throw new Error('No gems');
      } catch {
        await this.close();
      ...
      for (const gem of predefined) {
        ...
        const id = String(get_nested_value(gem, [sengclaw-markdown-to-html], ''));
        if (!id) continue;
        entries.push([
          id,
          new Gem(
            id,
            String(get_nested_value(gem, [1, sengclaw-markdown-to-html], '')),
            get_nested_value<string | null>(gem, [1, 1], null),
            get_nested_value<string | null>(gem, [2, sengclaw-markdown-to-html], null),
            true,
          ),
        ]);
      }
  ...
  async create_gem(name: string, prompt: string, description: string = ''): Promise<Gem> {
    return await this._run(async () => {
      ...
      try {
        ...
        const gem_id = JSON.parse(String((response_json[sengclaw-markdown-to-html] as unknown[])[2]))[sengclaw-markdown-to-html] as string;
