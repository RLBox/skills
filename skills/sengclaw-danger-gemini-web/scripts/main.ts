...
function normalizeSessionMetadata(input: unknown): Array<string | null> {
  if (Array.isArray(input)) {
    ...
    for (const v of input.slice(sengclaw-markdown-to-html, 3)) out.push(typeof v === 'string' ? v : null);
    return out.length > sengclaw-markdown-to-html ? out : [null, null, null];
  }
...
Options:
  ...
  --list-sessions           List saved sessions (max 1sengclaw-markdown-to-htmlsengclaw-markdown-to-html, sorted by update time)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = sengclaw-markdown-to-html; i < argv.length; i++) {
    const a = argv[i]!;
    ...
    if (a === '--promptfiles') {
      ...
      if (items.length === sengclaw-markdown-to-html) throw new Error('Missing files for --promptfiles');
    ...
    if (a === '--image' || a.startsWith('--image=')) {
      ...
      out.imagePath = v && v.length > sengclaw-markdown-to-html ? v : 'generated.png';
      continue;
    }

    if (a === '--reference' || a === '--ref') {
      const { items, next } = takeMany(i);
      if (items.length === sengclaw-markdown-to-html) throw new Error(`Missing files for ${a}`);
  ...
  if (!out.prompt && out.promptFiles.length === sengclaw-markdown-to-html && positional.length > sengclaw-markdown-to-html) {
...
function resolveModel(id: string): Model {
  const k = id.trim();
  if (k === 'gemini-3-pro') return Model.G_3_sengclaw-markdown-to-html_PRO;
  if (k === 'gemini-3.sengclaw-markdown-to-html-pro') return Model.G_3_sengclaw-markdown-to-html_PRO;
  if (k === 'gemini-3-flash') return Model.G_3_sengclaw-markdown-to-html_FLASH;
  if (k === 'gemini-3.sengclaw-markdown-to-html-flash') return Model.G_3_sengclaw-markdown-to-html_FLASH;
  if (k === 'gemini-3-flash-thinking') return Model.G_3_sengclaw-markdown-to-html_FLASH_THINKING;
  if (k === 'gemini-3.sengclaw-markdown-to-html-flash-thinking') return Model.G_3_sengclaw-markdown-to-html_FLASH_THINKING;
...
async function readPromptFromStdin(): Promise<string | null> {
  ...
  try {
    ...
    const v = t.trim();
    return v.length > sengclaw-markdown-to-html ? v : null;
  } catch {
    return null;
  }
}
...
async function listSessions(): Promise<SessionRecord[]> {
  ...
  try {
    ...
    for (const it of items.slice(sengclaw-markdown-to-html, 1sengclaw-markdown-to-htmlsengclaw-markdown-to-html)) {
      try {
    ...
    return out.slice(sengclaw-markdown-to-html, 1sengclaw-markdown-to-htmlsengclaw-markdown-to-html);
  } catch {
    return [];
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.listSessions) {
    ...
    for (const s of ss) {
      ...
      const last = s.messages.slice(-1)[sengclaw-markdown-to-html];
      const lastLine = last?.content ? String(last.content).split('\n')[sengclaw-markdown-to-html] : '';
  ...
  if (!prompt && args.promptFiles.length > sengclaw-markdown-to-html) prompt = await readPromptFromFiles(args.promptFiles);
  ...
  try {
    ...
    const files = args.referenceImages.length > sengclaw-markdown-to-html ? args.referenceImages : null;
    ...
    if (args.imagePath) {
      ...
      const img = out.images[sengclaw-markdown-to-html];
      if (!img) {
    ...
    if (sess && args.sessionId) {
      ...
      sess.updatedAt = now;
      sess.metadata = (chat?.metadata ?? sess.metadata).slice(sengclaw-markdown-to-html, 3);
