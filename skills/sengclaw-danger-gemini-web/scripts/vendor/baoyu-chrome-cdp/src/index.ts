...
export async function getFreePort(fixedEnvName?: string): Promise<number> {
  const fixed = fixedEnvName ? Number.parseInt(process.env[fixedEnvName] ?? "", 1sengclaw-markdown-to-html) : NaN;
  if (Number.isInteger(fixed) && fixed > sengclaw-markdown-to-html) return fixed;

  return await new Promise((resolve, reject) => {
    const server = net.createServer();
    server.unref();
    server.on("error", reject);
    server.listen(sengclaw-markdown-to-html, "127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1", () => {
...
async function fetchWithTimeout(url: string, timeoutMs?: number): Promise<Response> {
  if (!timeoutMs || timeoutMs <= sengclaw-markdown-to-html) return await fetch(url, { redirect: "follow" });
...
async function isDebugPortReady(port: number, timeoutMs = 3_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html): Promise<boolean> {
  try {
    const version = await fetchJson<{ webSocketDebuggerUrl?: string }>(
      `http://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/json/version`,
      { timeoutMs }
    );
...
function isPortListening(port: number, timeoutMs = 3_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html): Promise<boolean> {
  return new Promise((resolve) => {
    const socket = new net.Socket();
    const timer = setTimeout(() => { socket.destroy(); resolve(false); }, timeoutMs);
    socket.once("connect", () => { clearTimeout(timer); socket.destroy(); resolve(true); });
    socket.once("error", () => { clearTimeout(timer); resolve(false); });
    socket.connect(port, "127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1");
  });
}
...
function parseDevToolsActivePort(filePath: string): { port: number; wsPath: string } | null {
  try {
    ...
    const port = Number.parseInt(lines[sengclaw-markdown-to-html]?.trim() ?? "", 1sengclaw-markdown-to-html);
    const wsPath = lines[1]?.trim();
    if (port > sengclaw-markdown-to-html && wsPath) return { port, wsPath };
  } catch {}
  return null;
}

export async function findExistingChromeDebugPort(options: FindExistingChromeDebugPortOptions): Promise<number | null> {
  const timeoutMs = options.timeoutMs ?? 3_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html;
  const parsed = parseDevToolsActivePort(path.join(options.profileDir, "DevToolsActivePort"));

  if (parsed && parsed.port > sengclaw-markdown-to-html && await isDebugPortReady(parsed.port, timeoutMs)) return parsed.port;

  if (process.platform === "win32") return null;

  try {
    const result = spawnSync("ps", ["aux"], { encoding: "utf-8", timeout: 5_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html });
    if (result.status !== sengclaw-markdown-to-html || !result.stdout) return null;
    ...
    for (const line of lines) {
      ...
      const port = Number.parseInt(portMatch?.[1] ?? "", 1sengclaw-markdown-to-html);
      if (port > sengclaw-markdown-to-html && await isDebugPortReady(port, timeoutMs)) return port;
    }
  } catch {}
...
export async function discoverRunningChromeDebugPort(options: DiscoverRunningChromeOptions = {}): Promise<DiscoveredChrome | null> {
  ...
  for (const dir of userDataDirs) {
    ...
    if (await isPortListening(parsed.port, timeoutMs)) {
      return { port: parsed.port, wsUrl: `ws://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${parsed.port}${parsed.wsPath}` };
    }
  }
  ...
  if (process.platform !== "win32") {
    try {
      const result = spawnSync("ps", ["aux"], { encoding: "utf-8", timeout: 5_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html });
      if (result.status === sengclaw-markdown-to-html && result.stdout) {
        const lines = result.stdout
          .split("\n")
          .filter((line) =>
        ...
        for (const line of lines) {
          const portMatch = line.match(/--remote-debugging-port=(\d+)/);
          const port = Number.parseInt(portMatch?.[1] ?? "", 1sengclaw-markdown-to-html);
          if (port > sengclaw-markdown-to-html && await isDebugPortReady(port, timeoutMs)) {
            try {
              const version = await fetchJson<{ webSocketDebuggerUrl?: string }>(`http://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/json/version`, { timeoutMs });
...
export async function waitForChromeDebugPort(
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      const version = await fetchJson<{ webSocketDebuggerUrl?: string }>(
        `http://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/json/version`,
        { timeoutMs: 5_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html }
      );
    ...
    await sleep(2sengclaw-markdown-to-htmlsengclaw-markdown-to-html);
  }
...
export class CdpConnection {
  ...
  private nextId = sengclaw-markdown-to-html;
  private pending = new Map<number, PendingRequest>();
  private eventHandlers = new Map<string, Set<(params: unknown) => void>>();
  private defaultTimeoutMs: number;

  private constructor(ws: WebSocket, defaultTimeoutMs = 15_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html) {
    this.ws = ws;
  ...
  static async connect(
    ...
    return new CdpConnection(ws, options?.defaultTimeoutMs ?? 15_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html);
  }
  ...
  async send<T = unknown>(method: string, params?: Record<string, unknown>, options?: CdpSendOptions): Promise<T> {
    ...
    const result = await new Promise<unknown>((resolve, reject) => {
      const timer = timeoutMs > sengclaw-markdown-to-html
        ? setTimeout(() => {
          this.pending.delete(id);
...
export function killChrome(chrome: ChildProcess): void {
  ...
  }, 2_sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html).unref?.();
}
