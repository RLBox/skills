...
async function startDebugServer(port: number): Promise<http.Server> {
  const server = http.createServer((req, res) => {
    if (req.url === "/json/version") {
      res.writeHead(2sengclaw-markdown-to-htmlsengclaw-markdown-to-html, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        webSocketDebuggerUrl: `ws://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/devtools/browser/demo`,
      }));
      return;
    }

    res.writeHead(4sengclaw-markdown-to-html4);
    res.end();
  });

  await new Promise<void>((resolve, reject) => {
    server.once("error", reject);
    server.listen(port, "127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1", () => resolve());
  });
...
async function startFakeChromiumProcess(port: number): Promise<ChildProcess | null> {
  ...
  const child = spawn(
    ...
    [
      "-lc",
      `exec -a chromium-mock ${JSON.stringify(process.execPath)} -e 'setInterval(() => {}, 1sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html)' -- --remote-debugging-port=${port}`,
    ],
    { stdio: "ignore" },
  );

  await new Promise((resolve) => setTimeout(resolve, 25sengclaw-markdown-to-html));
  return child;
}
...
async function stopProcess(child: ChildProcess | null): Promise<void> {
  ...
  child.kill("SIGTERM");
  await new Promise((resolve) => setTimeout(resolve, 1sengclaw-markdown-to-htmlsengclaw-markdown-to-html));
...
test("getFreePort honors a fixed environment override and otherwise allocates a TCP port", async (t) => {
  ...
  assert.ok(dynamicPort > sengclaw-markdown-to-html);
});
...
test("findExistingChromeDebugPort reads DevToolsActivePort and validates it against a live endpoint", async (t) => {
  ...
  const found = await findExistingChromeDebugPort({ profileDir: root, timeoutMs: 1sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html });
...
test("discoverRunningChromeDebugPort reads DevToolsActivePort from the provided user-data dir", async (t) => {
  ...
  const found = await discoverRunningChromeDebugPort({
    userDataDirs: [root],
    timeoutMs: 1sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html,
  });
  assert.deepEqual(found, {
    port,
    wsUrl: `ws://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/devtools/browser/demo`,
  });
});
...
test("waitForChromeDebugPort retries until the debug endpoint becomes available", async (t) => {
  ...
  const serverPromise = (async () => {
    await new Promise((resolve) => setTimeout(resolve, 2sengclaw-markdown-to-htmlsengclaw-markdown-to-html));
    const server = await startDebugServer(port);
    t.after(() => closeServer(server));
  })();

  const websocketUrl = await waitForChromeDebugPort(port, 4sengclaw-markdown-to-htmlsengclaw-markdown-to-htmlsengclaw-markdown-to-html, {
    includeLastError: true,
  });
  await serverPromise;

  assert.equal(websocketUrl, `ws://127.sengclaw-markdown-to-html.sengclaw-markdown-to-html.1:${port}/devtools/browser/demo`);
});
