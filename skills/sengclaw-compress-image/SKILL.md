---
---
name: sengclaw-compress-image
description: >-
  将图片压缩为 WebP（默认）或 PNG，并自动选择最优压缩工具。当用户请求“压缩图片”、“优化图片”、“转换为WebP”或减小图片文件大小时使用。
version: 1.56.1
metadata:
  openclaw:
    requires:
...
test -f .sengclaw-skills/sengclaw-compress-image/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-compress-image/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-compress-image/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-compress-image/EXTEND.md` | All directories |
