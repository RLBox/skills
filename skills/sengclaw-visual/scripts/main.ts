...
Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.sengclaw-skills/.env > ~/.sengclaw-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".sengclaw-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".sengclaw-skills", ".env"));
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        const cleaned = value.replace(/['"]/g, "");
        config.default_scene = cleaned === "null" ? null : cleaned;
      } else if (key === "prompt_forbidden") {
        const cleaned = value.replace(/['"]/g, "");
        config.prompt_forbidden = cleaned === "null" ? null : cleaned;
      } else if (key === "watermark") {
        // Top-level watermark block
        if (value === "null") {
          config.watermark = null;
        } else {
          config.watermark ??= { text: null };
        }
        currentKey = "watermark";
        currentProvider = null;
      } else if (currentKey === "watermark" && indent >= 2 && key === "text") {
        config.watermark ??= { text: null };
        const cleaned = value.replace(/['"]/g, "");
        config.watermark.text = cleaned === "null" ? null : cleaned;
      } else if (currentKey === "watermark" && indent >= 2 && key === "scene_overrides") {
        config.watermark ??= { text: null };
        config.watermark.scene_overrides ??= {};
        currentKey = "watermark_scene_overrides";
      } else if (currentKey === "watermark_scene_overrides" && indent >= 4) {
        config.watermark ??= { text: null };
        config.watermark.scene_overrides ??= {};
        const cleaned = value.replace(/['"]/g, "");
        if (cleaned === "false" || cleaned === "null") {
          config.watermark.scene_overrides[key] = false;
        } else {
          config.watermark.scene_overrides[key] = cleaned;
        }
      } else if (key === "batch") {
        config.batch = {};
        currentKey = "batch";
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(cwd, ".sengclaw-skills", "sengclaw-image-gen", "EXTEND.md"),
    path.join(home, ".sengclaw-skills", "sengclaw-image-gen", "EXTEND.md"),
  ];
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, or ARK_API_KEY.\n" +
      "Create ~/.sengclaw-skills/.env or <cwd>/.sengclaw-skills/.env with your keys."
  );
}
...
async function prepareSingleTask(args: CliArgs, extendConfig: Partial<ExtendConfig>): Promise<PreparedTask> {
  ...
  const watermarkText = resolveWatermarkText(extendConfig.watermark, extendConfig.default_scene ?? null);
...
/**
 * Resolve effective watermark text for a given scene.
 * - scene_overrides[scene] === false/null → disabled
 * - scene_overrides[scene] === string → use that text
 * - otherwise fall back to watermark.text
 * Returns null if watermark should be skipped.
 */
export function resolveWatermarkText(
  watermark: import("./types").WatermarkConfig | null | undefined,
  scene: string | null
): string | null {
  if (!watermark) return null;
  if (scene && watermark.scene_overrides) {
    const override = watermark.scene_overrides[scene];
    if (override === false || override === null) return null;
    if (typeof override === "string" && override.trim()) return override.trim();
  }
  return watermark.text?.trim() || null;
}

/**
 * Apply text watermark to an image file using Python PIL.
 * Overlays text at bottom-right corner with semi-transparent background.
 */
async function applyWatermark(imagePath: string, text: string): Promise<void> {
  const script = `
import sys
...
# Determine font size relative to image width
...
rx1 = w - tw - padding * 2 - margin
ry1 = h - th - padding * 2 - margin
rx2 = w - margin
ry2 = h - margin

# Draw semi-transparent rounded background
draw.rectangle([rx1, ry1, rx2, ry2], fill=(0, 0, 0, 120))
...
if ext in ("jpg", "jpeg"):
    ...
    combined.save(img_path, "JPEG", quality=95)
else:
...
`.trim();
  ...
  await new Promise<void>((resolve, reject) => {
    const proc = spawn("python3", ["-c", script, imagePath, text]);
    const stderr: string[] = [];
    proc.stderr.on("data", (d: Buffer) => stderr.push(d.toString()));
    proc.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`Watermark failed (exit ${code}): ${stderr.join("")}`));
    });
  });
}
...
async function generatePreparedTask(task: PreparedTask): Promise<TaskResult> {
  ...
  while (attempts < MAX_ATTEMPTS) {
    ...
    try {
      ...
      if (task.watermarkText) {
        await applyWatermark(task.outputPath, task.watermarkText);
        console.error(`[${task.id}] Watermark applied: "${task.watermarkText}"`);
      }
      return {
        id: task.id,
