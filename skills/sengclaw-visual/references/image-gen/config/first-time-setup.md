...
description: First-time setup and default model selection flow for sengclaw-image-gen
---
...
options:
  - label: "Project (Recommended)"
    description: ".sengclaw-skills/ (this project only)"
  - label: "User"
    description: "~/.sengclaw-skills/ (all projects)"
```
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.sengclaw-skills/sengclaw-image-gen/EXTEND.md` | Current project |
| User | `$HOME/.sengclaw-skills/sengclaw-image-gen/EXTEND.md` | All projects |
...
- In `sengclaw-image-gen`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `sengclaw-image-gen`, `quality` is a compatibility preset. It is not a native DashScope parameter.
