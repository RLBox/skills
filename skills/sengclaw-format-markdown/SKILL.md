---
---
name: sengclaw-format-markdown
description: >-
  为纯文本或 Markdown 文件添加 frontmatter、标题、摘要、小标题、加粗、列表和代码块等格式。当用户说"格式化 Markdown"、"美化文章"、"加排版"或需要改善文章结构时使用。输出文件名为 {文件名}-formatted.md。
version: 1.57.0
metadata:
  openclaw:
    requires:
...
test -f .sengclaw-skills/sengclaw-format-markdown/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-format-markdown/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-format-markdown/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-format-markdown/EXTEND.md` | All directories |
