---
name: sengclaw-slide-deck
description: 根据内容生成专业幻灯片图片。先创建带风格说明的大纲，再逐张生成幻灯片图片。当用户说"制作幻灯片"、"做演示文稿"、"生成 PPT"、"slide deck"、"幻灯片"、"参考图做PPT"、"仿制PPT"、"风格参考"、"抄这个风格"、"模仿这个排版"时使用。
version: 1.57.0
metadata:
  openclaw:
    requires:
...
```bash
/sengclaw-slide-deck path/to/content.md
/sengclaw-slide-deck path/to/content.md --style sketch-notes
/sengclaw-slide-deck path/to/content.md --audience executives
/sengclaw-slide-deck path/to/content.md --lang zh
/sengclaw-slide-deck path/to/content.md --slides 10
/sengclaw-slide-deck path/to/content.md --outline-only
/sengclaw-slide-deck  # Then paste content

# Reference-Driven Mode (四抄法) — see references/reference-driven-guide.md
/sengclaw-slide-deck path/to/content.md --ref-subject path/to/subject.jpg
/sengclaw-slide-deck path/to/content.md --ref-style path/to/style-poster.jpg
/sengclaw-slide-deck path/to/content.md --ref-layout path/to/layout-sketch.jpg
/sengclaw-slide-deck path/to/content.md --ref-subject s.jpg --ref-style r.jpg --ref-layout l.jpg
```
...
| `--ref-subject <path>` | **[四抄法-抄主体]** Reference image of the main subject/product; AI generates multi-angle views |
| `--ref-style <path>` | **[四抄法-抄风格]** Reference poster/design image; AI extracts color, mood, composition |
| `--ref-layout <path>` | **[四抄法-抄布局]** Hand-drawn layout sketch or wireframe; AI follows this spatial arrangement |
...
- [ ] Step 1: Setup & Analyze
  ...
  - [ ] 1.4 Collect reference materials (if any) — 四抄法
...
test -f .sengclaw-skills/sengclaw-slide-deck/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-slide-deck/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-slide-deck/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-slide-deck/EXTEND.md` | All directories |
...
### Step 1.4 Collect Reference Materials (四抄法)

**Purpose**: Upgrade slide quality by learning from real-world references — subject photos, style posters, layout sketches.

Full workflow details: `references/reference-driven-guide.md`

**Trigger conditions** (check at least one):
- User passed `--ref-subject`, `--ref-style`, or `--ref-layout` flags
- User mentioned "参考图", "风格参考", "仿制", "抄这个风格", "按这个排版", or attached images in conversation
- User described their subject/product visually (implies subject reference needed)

**If no reference flags and no mention** → Skip this step; proceed to Step 2.

**If triggered**, for each reference type provided:

#### 1.4A Collect — 抄主体 (Subject Reference)

When `--ref-subject <path>` is provided OR user describes a specific product/subject:

```
Reference type: Subject (主体)
File: [path or "described by user"]
Purpose: Generate multi-angle views of the subject for use as slide visual assets
```

**Agent action**: Note file path in `analysis.md` under `ref_subject`. During Step 5 (prompt generation), inject subject reference instruction.

#### 1.4B Collect — 抄风格 (Style Reference)

When `--ref-style <path>` is provided OR user says "按这个风格/仿制这个":

```
Reference type: Style (风格)
File: [path]
Purpose: Extract color palette, composition mood, typography feeling, atmosphere
```

**Agent action**:
1. Analyze the reference image (describe it in `analysis.md` under `ref_style_analysis`):
   - Dominant colors (with approximate hex)
   - Composition pattern (e.g., centered, rule-of-thirds, asymmetric)
   - Mood/atmosphere keywords (e.g., dark+cinematic, clean+minimalist, vibrant+energetic)
   - Typography character (bold/light, serif/sans, display/body)
2. This analysis **overrides** or **supplements** the preset style dimensions in `STYLE_INSTRUCTIONS`
3. Save extracted style keywords to `analysis.md`

#### 1.4C Collect — 抄布局 (Layout Reference)

When `--ref-layout <path>` is provided OR user says "按这个布局/草图":

```
Reference type: Layout (布局)
File: [path]
Purpose: Follow the spatial arrangement in the sketch for slide composition
```

**Agent action**: Note file path in `analysis.md` under `ref_layout`. During Step 5 (prompt generation), inject layout sketch instruction for each matching slide.

#### 1.4D Save Reference Summary

Append to `analysis.md`:

```markdown
## Reference Materials (四抄法)

### 抄主体 (Subject Reference)
- File: [path or "none"]
- Description: [brief description of what the subject is]
- Usage: Inject into visual prompts for main subject consistency

### 抄风格 (Style Reference)
- File: [path or "none"]
- Extracted Colors: [color1 #hex, color2 #hex, ...]
- Composition: [keywords]
- Mood: [keywords]
- Typography: [keywords]
- Usage: Override STYLE_INSTRUCTIONS color + atmosphere sections

### 抄布局 (Layout Reference)
- File: [path or "none"]
- Layout description: [describe what areas/grid the sketch defines]
- Usage: Inject as layout constraint into slide prompts
```
...
3. **If Reference Materials were collected in Step 1.4**, inject reference instructions per slide:
   - **抄主体**: Add `SUBJECT_REFERENCE` block — describe the subject from `ref_subject` image, instruct AI to render it from the appropriate angle for this slide
   - **抄风格**: Add `STYLE_OVERRIDE` block — inject extracted colors, mood, composition from `ref_style_analysis`; override the standard STYLE_INSTRUCTIONS palette/atmosphere sections
   - **抄布局**: Add `LAYOUT_REFERENCE` block — describe the spatial arrangement from `ref_layout` sketch; instruct AI to follow that grid/composition
   - See `references/reference-driven-guide.md` for exact prompt injection templates
...
```bash
/sengclaw-slide-deck content.md --prompts-only
```
...
```bash
/sengclaw-slide-deck slide-deck/topic-slug/ --images-only
```
...
```bash
# Single slide
/sengclaw-slide-deck slide-deck/topic-slug/ --regenerate 3

# Multiple slides
/sengclaw-slide-deck slide-deck/topic-slug/ --regenerate 2,5,8
```
...
2. Run: `/sengclaw-slide-deck <dir> --regenerate N`
...
| `references/reference-driven-guide.md` | **四抄法** reference-driven mode — subject/style/layout injection |
