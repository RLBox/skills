...
<!-- REFERENCE_MATERIALS block: only include if Step 1.4 collected references -->
<REFERENCE_MATERIALS>
Subject Reference:
  File: [path or "none"]
  Description: [what the subject is, key identifying features]
  Angles per slide type:
    Cover: [recommended angle]
    Feature: [recommended angle]
    Detail: [recommended angle]
    Back Cover: [recommended angle]
...
Style Reference:
  ...
  Mood: [keywords]
  Typography feel: [headline / body description]
  Override: [which STYLE_INSTRUCTIONS sections this overrides]

Layout Reference:
  File: [path or "none"]
  Zone description: [zone-by-zone layout from sketch]
  Applicable slides: [all / specific types]
</REFERENCE_MATERIALS>
...
// REFERENCE INJECTION (only if REFERENCE_MATERIALS block exists in outline)
Subject: [angle to render the subject from REFERENCE_MATERIALS.Subject]
Style Override: [yes/no — apply extracted color+mood from REFERENCE_MATERIALS.Style]
Layout Override: [yes/no — follow sketch layout from REFERENCE_MATERIALS.Layout]
```
