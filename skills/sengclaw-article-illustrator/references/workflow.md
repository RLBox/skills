...
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/sengclaw-skills/sengclaw-article-illustrator/EXTEND.md" && echo "xdg"
test -f "$HOME/.sengclaw-skills/sengclaw-article-illustrator/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .sengclaw-skills/sengclaw-article-illustrator/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/sengclaw-skills/sengclaw-article-illustrator/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.sengclaw-skills/sengclaw-article-illustrator/EXTEND.md") { "user" }
```
...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer `sengclaw-image-gen` batch mode (`build-batch.ts` -> `main.ts --batchfile`)
...
|------------------------|--------|
| Yes (e.g., sengclaw-image-gen with Google) | Pass reference images via `--ref` |
