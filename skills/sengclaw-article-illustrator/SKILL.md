---
name: sengclaw-article-illustrator
description: '为文章生成配图。分析文章结构，识别需要配图的位置，通过「类型×风格」双维度生成风格统一的插图、信息图、流程图等。当用户说「配图」、「生成文章图片」、「为文章添加图片」、「illustrate article」或「为文章配图」时使用。'
version: 1.56.1
metadata:
  openclaw:
---
...
test -f .sengclaw-skills/sengclaw-article-illustrator/EXTEND.md && echo "project"
# 2. User level (Clacky workspace — applies across all directories)
test -f "$HOME/clacky_workspace/.sengclaw-skills/sengclaw-article-illustrator/EXTEND.md" && echo "user"
```

| Level | Path | Scope |
|-------|------|-------|
| Project | `.sengclaw-skills/sengclaw-article-illustrator/EXTEND.md` | Current directory only |
| User (default) | `~/clacky_workspace/.sengclaw-skills/sengclaw-article-illustrator/EXTEND.md` | All directories |

| Result | Action |
|--------|--------|
| Found | Read, parse, display summary |
| Not found | ⛔ Run [first-time-setup](references/config/first-time-setup.md) → Save to user-level path → Continue |
...
**Execution strategy**: When multiple illustrations have saved prompt files and the task is now plain generation, prefer `sengclaw-image-gen` batch mode (`build-batch.ts` → `--batchfile`) over spawning subagents. Use subagents only when each image still needs separate prompt iteration or creative exploration.
