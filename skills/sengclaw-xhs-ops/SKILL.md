---
name: sengclaw-xhs-ops
description: "小红书相关操作，覆盖账号定位、选题研究、内容生产、发布执行与复盘修复的小红书全链路运营技能。内置对标账号深度分析能力（搜索账号→抓取真实数据→生成竞品报告），当用户说「找对标账号」「竞品分析」「分析小红书账号」「对标分析」「帮我分析这几个账号」时也会触发。内置评论导流能力（主动去爆款帖子评论区埋钩子涨粉），当用户说「评论导流」「评论钩子」「帮我写评论」「去别人帖子引流」「导流评论」时也会触发。凡是小红书的浏览/搜索/发布/评论任务，默认必须使用 OpenClaw 内置浏览器流程并指定 profile=\"openclaw\"；除非用户明确要求，否则不要使用系统 open 或外部浏览器。"
---
...
**每次任务启动时，先检查 EXTEND.md 是否存在：**

```
优先：~/clacky_workspace/.sengclaw-skills/sengclaw-xhs-ops/EXTEND.md
备选：.sengclaw-skills/sengclaw-xhs-ops/EXTEND.md（当前目录项目级覆盖）
```

- **存在** → 读取配置，后续所有模板/话术用其中的变量动态填充（`{nickname}` `{background}` `{product}` `{signature}` 等）
- **不存在** → 按 `references/config/first-time-setup.md` 引导用户完成首次配置后再继续

配置 schema 完整说明：`references/config/preferences-schema.md`
...
### A.0 对标账号深度分析（新增，融合自 sengclaw-xhs-analysis）

按 `references/xhs-competitor-analysis.md` 执行完整流程。

**触发条件**：用户说「找对标账号」「分析竞品」「对标分析」「xhs competitor」「帮我分析这几个账号」时，优先走此流程。

**核心技术要点**（必须遵守）：
- 直接用数字小红书号拼 URL 会报「未连接到服务器」
- 必须先从搜索页 `https://www.xiaohongshu.com/search_result?keyword=账号名&type=user` 抓取带 `xsec_token` 的完整 URL，再导航进入主页
- 抓取 JS：
```js
JSON.stringify(Array.from(document.querySelectorAll('a[href*="user/profile"]'))
  ...
  .slice(0, 10))
```
- 进入主页后用 `document.body.innerText.substring(0, 4000)` 抓取数据
- 每个账号：采样最近 9-15 篇笔记，重点记录标题 + 赞藏数
- 报告保存路径：`~/clacky_workspace/xhs-competitor-analysis-YYYY-MM-DD.md`

**图文专项**：分析时特别标注哪些账号以图文为主、哪些爆款是图文格式（为图文内容策略提供直接参考）。
...
**触发词**：「评论导流」「评论区引流」「去别人帖子评论」「评论钩子」「评论模板」「帮我写评论」「导流评论」时，走此流程。

完整方法论见 `references/xhs-comment-diversion.md`，执行步骤如下：

### 执行流程
1. **选区**：按方法论 Section 2 标准，在平台搜索目标领域爆款笔记（点赞≥1000，≤7天）
2. **钩子选型**：根据笔记情绪基调选择对应类型：
   - A 型（身份植入）：适合中性/共鸣类内容
   - B 型（悬念钩子）：适合干货/教程类内容
   - C 型（反差对比）：适合励志/逆袭/焦虑类内容
   - D 型（福利赠品）：有明确资料包时使用
3. **生成评论文案**：基于模板微调，每条都必须包含身份锚点
4. **发布**：按风控节奏（1小时≤3条），不复制粘贴相同话术
5. **记录**：发布后记录帖子 URL + 评论内容，次日复查点击转化

### 快速出评模式（用户明确要求时）
直接输出 3 条可即用的钩子评论（覆盖 A/B/C 三型），格式：
```
[A-身份植入]：xxx
[B-悬念钩子]：xxx  
[C-反差对比]：xxx
