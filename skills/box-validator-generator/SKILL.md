...
| 模块 | 用途 | 典型任务 | 涉及模型 |
|---|---|---|---|
| `post` | 发布/编辑/删除闲置帖 | "发布一条价格 5000+ 元的 iPhone 13 Pro 帖子" | Post, Category, Brand |
| `order` | 买卖交易、支付 | "购买帖子中的商品" | Order, Post, Address |
| `message` | 站内私信 | "给卖家发询价消息" | Conversation, Message, Post |
| `comment` | 帖子评论 | "给帖子写一条评论" | Comment, Post |
| `favorite` | 收藏 | "收藏这个商品" | Favorite, Post |
| `like` | 点赞（蹲蹲） | "蹲蹲这个帖子" / "取消蹲蹲" | Like, Post |
| `search` | 搜索 | "搜索「耳机」" | Post |
| `address` | 收货地址 | "新增深圳收货地址" | Address |
| `user` | 用户资料 | "修改昵称" | User |
| `support` | 客服 | — | — |
...
| 类型 | 查询方式 |
|---|---|
| Demo 用户（张三） | `User.find_by!(email: 'demo@rlbox.ai', data_version: '0')`（密码 `password123`） |
| 其他用户（李四） | `User.find_by!(email: 'lisi@example.com', data_version: '0')` |
...
**IdleSwap 专属规则（Zeitwerk + 跨请求隔离）**：

1. **命名空间必须是 `Validators::{Module}::V{NNN}{Module}Validator`**（Zeitwerk 自动加载，不需要 `require_relative`）
2. **必须实现 `execution_state_data` + `restore_from_state`**（ADR-006，prepare 和 verify 是两次独立 HTTP 请求）
3. **verify 里用 `user_ids_for(@user)` 兼容 sandbox user**（BaseValidator helper，返回 `[sandbox_user_id, baseline_user_id]`）
4. **simulate 不传 data_version**（DataVersionable 自动从 SESSION 变量读取）

```ruby
# IdleSwap 专属：跨请求状态持久化
...
# IdleSwap 专属：user_ids_for 兼容 sandbox user
def verify
...
# ❌ 反例 K：IdleSwap 里没有实现 execution_state_data / restore_from_state
# → verify 是新的 HTTP 请求，@user 等实例变量全是 nil → NPE
def verify
  Comment.where(user: @user, ...)   # @user == nil
end
# ✅ 正确：必须实现两个方法
def execution_state_data
  { user_id: @user.id }
...
# ❌ 反例 L：IdleSwap verify 里 user 查询不用 user_ids_for
def verify
  @comment = Comment.where(user: @user, data_version: @data_version).first
...
- **模板骨架（Goomart / 通用）**：`TEMPLATE_GOOMART.rb`（本目录下，带 seed + load_refs，适用多项目）
- **模板骨架（IdleSwap 专属）**：`TEMPLATE_IDLESWAP.rb`（本目录下，带 execution_state_data / restore_from_state）
